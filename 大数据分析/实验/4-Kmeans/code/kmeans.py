...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        plt.scatter(cluster_data[:, 11], cluster_data[:, 9], 
                    c=colors[i], marker=markers[i], label=f'Cluster {i+1}', alpha=0.7, edgecolors='k')
        plt.scatter(centroids[i, 11], centroids[i, 9], 
                    c=colors[i], marker='X', s=250, linewidths=2, 
                    edgecolors='black', label=f'Centroid {i+1}')
    plt.xlabel('Feature 12 (Normalized)', fontsize=12)  # X轴标签
    plt.ylabel('Feature 10 (Normalized)', fontsize=12)  # Y轴标签
